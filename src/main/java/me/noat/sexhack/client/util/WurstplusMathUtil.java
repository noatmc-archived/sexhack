...
class WurstplusMathUtil {
    ...
    public static
    double[] directionSpeedNoForward(double speed) {
    ...
    public static
    double linear(double from, double to, double incline) {
    ...
    public static
    double getDistance(Vec3d pos, double x, double y, double z) {
    ...
    public static
    double[] calcIntersection(double[] line, double[] line2) {
    ...
    public static
    int clamp(final int num, final int min, final int max) {
        return (num < min) ? min : Math.min(num, max);
    }

    public static
    float clamp(final float num, final float min, final float max) {
        return (num < min) ? min : Math.min(num, max);
    }

    public static
    double clamp(final double num, final double min, final double max) {
        return (num < min) ? min : Math.min(num, max);
    }
    ...
    public static
    Vec3d roundVec(final Vec3d vec3d, final int places) {
        return new Vec3d(round(vec3d.x, places), round(vec3d.y, places), round(vec3d.z, places));
    }

    public static
    double square(final double input) {
        return input * input;
    }

    public static
    double round(final double value, final int places) {
        if (places < 0) {
    ...
    public static
    Vec3d direction(final float yaw) {
        return new Vec3d(Math.cos(degToRad(yaw + 90.0f)), 0.0, Math.sin(degToRad(yaw + 90.0f)));
    }

    public static
    float round(final float value, final int places) {
        if (places < 0) {
            throw new IllegalArgumentException();
        }
        BigDecimal bd = BigDecimal.valueOf(value);
    ...
    public static
    double radToDeg(final double rad) {
        return rad * 57.295780181884766;
    }

    public static
    double degToRad(final double deg) {
    ...
    public static
    double[] directionSpeed(final double speed) {
    ...
    public static
    List <Vec3d> getBlockBlocks(final Entity entity) {
    ...
    public static
    boolean areVec3dsAlignedRetarded(final Vec3d vec3d1, final Vec3d vec3d2) {
    ...
    public static
    float[] calcAngle(final Vec3d from, final Vec3d to) {
