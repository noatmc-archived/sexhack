...
public
class WurstplusAutoArmour extends Module {

    final Setting delay = create("Delay", "AADelay", 2, 0, 5);
    final Setting smart_mode = create("Smart Mode", "AASmartMode", true);
    final Setting put_back = create("Equip Armour", "AAEquipArmour", true);
    final Setting player_range = create("Player Range", "AAPlayerRange", 8, 0, 20);
    final Setting crystal_range = create("Crystal Range", "AACrystalRange", 13, 0, 20);
    final Setting boot_percent = create("Boot Percent", "AATBootPercent", 80, 0, 100);
    final Setting chest_percent = create("Chest Percent", "AATChestPercent", 80, 0, 100);
    ...
    private static
    Map <Integer, ItemStack> get_inv() {
        return get_inv_slots(9, 44);
    }

    private static
    Map <Integer, ItemStack> get_armour() {
    ...
    void update() {
        ...
        if (smart_mode.getValue(true)) {
            if (!(is_crystal_in_range(crystal_range.getValue(1)) || is_player_in_range(player_range.getValue(1))))
                flag = true;
        }
    ...
    public
    boolean is_player_in_range(int range) {
    ...
    public
    boolean is_crystal_in_range(int range) {
    ...
    public
    boolean is_healed(ItemStack item) {
